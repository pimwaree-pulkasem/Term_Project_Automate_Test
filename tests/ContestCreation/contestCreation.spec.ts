...
test.describe('Contest Creation', () => {
  ...
  test('Contest-Creation-TC-02| แจ้งเตือนเมื่อไม่กรอกรายละเอียดการประกวด', async ({ page }) => {
    const contestPage = new ContestPage(page);
    await contestPage.goToCreateContest();
    await contestPage.fillContestForm({ name: 'การประกวดแต่งกลอนแปด ระดับประเทศ ประจำปี 2026', detail: '', endDate: FUTURE_DATE });
    await contestPage.submit();
  ...
  test('Contest-Creation-TC-03 | แจ้งเตือนเมื่อไม่กรอกชื่อการประกวด', async ({ page }) => {
    const contestPage = new ContestPage(page);
    await contestPage.goToCreateContest();
    await contestPage.fillContestForm({ name: '', detail: 'รายละเอียด',
      rules: 'ห้ามใช้ AI', endDate: FUTURE_DATE, scoreWeight: 100});
    await contestPage.submit();
  ...
  test('Contest-Creation-TC-04 | แจ้งเตือนเมื่อระบุวันสิ้นสุดเป็นอดีต', async ({ page }) => {
    const contestPage = new ContestPage(page);
    await contestPage.goToCreateContest();
    await contestPage.fillContestForm({ name: 'การประกวดแต่งกลอนแปด ระดับประเทศ ประจำปี 2026', detail: 'รายละเอียด',
  ...
  test('Contest-Creation-TC-05 | แสดงสถานะ "สร้างสำเร็จ" หลังจากสร้างการประกวด', async ({ page }) => {
    ...
    const contestPage = new ContestPage(page);
    await contestPage.goToCreateContest();
    await contestPage.fillContestForm({   name: contestName , detail: 'รายละเอียด',
      rules: 'ห้ามใช้ AI', endDate: FUTURE_DATE, scoreWeight: 100,
    });
    await contestPage.submit();
  ...
  test('Contest-Creation-TC-09 | ควรเด้งไปหน้า Login และไม่บันทึกข้อมูลเมื่อ Session หมดอายุ', async ({ page }) => {
    const contestPage = new ContestPage(page);
    await contestPage.goToCreateContest();
    await contestPage.fillContestForm({ name:'การประกวดแต่งกลอนแปด ระดับประเทศ ประจำปี 2026', detail: 'รายละเอียด',
      rules: 'ห้ามใช้ AI', endDate: FUTURE_DATE, scoreWeight: 100 });
